...
import { generateReconciliationPDF } from "../utils/reconciliationAct";
...
export default class UserHandler {
  ...
  public async handleUserRegistration(chatId: number) {
    ...
    await this.bot.sendMessage(chatId, i18n.t("choose_language"), {
      reply_markup: {
    ...
    this.bot.once("message", async (languageResponse: TelegramBot.Message) => {
      ...
      if (language) {
        ...
        if (["🇷🇺Русский", "🇺🇸English", "🇺🇿Uzbek"].includes(language)) {
          ...
          await this.bot.sendMessage(chatId, i18n.t("share_contact"), {
            reply_markup: {
  ...
  public async handleContact(msg: TelegramBot.Message) {
    ...
    if (phoneNumber && name) {
      ...
      if (existingUser) this.bot.sendMessage(chatId, i18n.t("user_already_exists"));
      else {
  ...
  public async handleMyCreditCard(msg: TelegramBot.Message) {
    ...
    if (user) {
      ...
      const lastTransactions = transactions
        ...
        .map((transaction: any) => {
          ...
          return `${
            transaction.sum > 0 ? i18n.t("bonuses_addition") : i18n.t("bonuses_removal")
    ...
    } else this.bot.sendMessage(chatId, i18n.t("user_not_found"));
  }
  ...
  public async handleAddAdmin(msg: TelegramBot.Message) {
    ...
    try {
      ...
      this.bot.onReplyToMessage(chatId, replyMessage.message_id, async (replyMsg) => {
        ...
        } else await this.bot.sendMessage(chatId, i18n.t("admin_not_found"));
  ...
  public async handleRemoveAdmin(msg: TelegramBot.Message) {
    ...
    try {
      ...
      this.bot.onReplyToMessage(chatId, replyMessage.message_id, async (replyMsg) => {
        ...
        // NOTE: move this method to UTILS
  ...
  public async handleReconciliationAct(msg: TelegramBot.Message) {
    ...
    this.bot.onReplyToMessage(
      msg.chat.id,
      startDateForceReplyMsg.message_id,
      async (startReply) => {
        ...
        if (await this.isValidDate(startDate)) {
          ...
          this.bot.onReplyToMessage(
            msg.chat.id,
            endDateForceReplyMsg.message_id,
            async (endReply) => {
              ...
              if (await this.isValidDate(endDate)) {
                ...
                const filteredTransactions = allTransactions.filter((transaction) => {
                  const transactionDate = moment(transaction.date, "DD.MM.YYYY").format(
                    "YYYY-MM-DD",
                  );
                ...
                const pdfPath = path.resolve(
                  tempDir,
                  `reconciliationAct_${msg.chat.id}_${startDate}_${endDate}.pdf`,
                );
  ...
  // NOTE: move this method to something like utils
  // date validation
